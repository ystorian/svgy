...
//!
//! The output (a) rewrites the root `viewBox`, (b) drops the root `width`/`height`,
//! and (c) uses **no** `transform` to scale — the factor is multiplied into every
//! coordinate/length in place, preserving the original element structure. A uniform
//! scale is origin-independent and commutes with rotation, so this renders
//! identically to the source while changing only its intrinsic (viewBox) size.
...
fn process_element(
    ...
    // Gradient/pattern coordinates are only in user space (and thus scalable)
    // when *Units="userSpaceOnUse"; the default objectBoundingBox uses 0..1
    // ratios that must be left untouched (as must the matching *Transform).
...
fn scale_attr(key: &str, val: &str, s: f64) -> String {
    match key {
        ...
        | "x2" | "y2" | "dx" | "dy" | "fx" | "fy" | "stroke-width" | "stroke-dashoffset"
...
/// Scale the translation parts of a `transform` list without adding transforms.
/// For a uniform scale S: `translate(tx,ty)` -> both scale; `matrix(a b c d e f)`
/// -> only e,f scale; `rotate(a[,cx,cy])` -> center scales, angle does not;
/// `scale/skewX/skewY` -> unchanged.
...
/// Scale all coordinates in path `d` data. Arc rx/ry/endpoint scale; the
/// x-axis-rotation and the large-arc/sweep flags do not. On any parse error the
