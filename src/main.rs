...
// Geometry reads better with the names the maths uses: a circle is (cx, cy, r),
// a triangle is (a, b, c), and spelling those out obscures more than it helps.
...
fn cmd_png_size(tree: &usvg::Tree) -> (u32, u32) {
    // Rounding to whole pixels is the point of the cast, and `max(1.0)` keeps
    // the value positive before it is taken.
...
fn write_png(tree: &usvg::Tree, out: &Path, optimize_png: bool) -> Result<()> {
    // The tree already carries the requested size, so the PNG is its intrinsic
    // size: no letterbox, and `--size` means the same thing it does for the SVG.
...
/// Where a target writes: the explicit destination if given, else the source
/// name with `suffix` and `ext` beside it — `logo.svg` -> `logo.svgy.png`.
///
/// A destination may name a directory that does not exist yet, as the
/// icons-into-subfolders example does, so the parent is created here rather
/// than failing partway through a multi-target run.
fn resolve_output(
    cli: &Cli,
    dest: Option<&Path>,
    suffix: &str,
...
mod tests {
    ...
    /// `--svg` and `--round` both write an SVG, so `--round` uses its own
    /// suffix and the two never claim the same path.
    #[test]
    ...
    fn custom_suffix_does_not_move_round() {
        let input = Path::new("logo.svg");
        assert_eq!(
            default_name(input, "v2", "svg"),
            Path::new("logo.v2.svg")
        );
        assert_eq!(
    ...
    /// `--size` fits the longest side for the SVG and the PNG alike: a 200x100
    /// source becomes 1024x512, not a padded 1024x1024.
    #[test]
    fn size_fits_the_longest_side_for_both_outputs() {
        ...
        let resized =
            svg_resize::transform_svg(src, cli::Target::Square(1024)).unwrap();
    ...
    fn width_and_height_fit_inside_the_box() {
        let src = r##"<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 100"><rect width="200" height="100" fill="#000"/></svg>"##;
        let resized =
            svg_resize::transform_svg(src, cli::Target::Both(300, 300)).unwrap();
        let tree = render::load_tree_from_data(resized.as_bytes()).unwrap();
