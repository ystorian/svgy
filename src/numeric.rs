//! Numeric formatting and length/number parsing shared by the coordinate-baking
//! commands (`svg` resize and `round`).
...
/// Apply an affine `s*n + t` to a single length (position component); only
/// unitless and `px` values are transformed.
