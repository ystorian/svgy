...
//!
//! All rendering preserves aspect ratio and fits the drawing inside the target
//! box (transparent letterbox padding), matching `rsvg-convert --keep-aspect-ratio`.
...
/// Render the tree into a `target_w` x `target_h` pixmap, scaling to fit inside
/// while preserving aspect ratio and centering the result.
// Icon sides are small enough to be exact in an f32, and the pixmap allocation
// below is what would fail on an absurd one.
...
/// Render the tree at a uniform scale `k` into a pixmap sized to the scaled
/// intrinsic size, with no translation. Device pixel (x, y) therefore maps to
/// tree coordinate (x/k, y/k) exactly — used for geometric measurement.
// Rounding the scaled size up to whole pixels is the point of the cast; `max`
// keeps it positive, and `as` saturates rather than wrapping.
