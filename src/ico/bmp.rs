...
//!
//! Windows picks the entry whose declared depth best matches the display, so a
//! session capped at 256 colors (old `mstsc`, or the "Limit maximum color depth"
//! policy) wants an entry declaring 8 bits per pixel; PNG entries declare 32.
//! Pre-Vista shells cannot read PNG entries at all, so these also keep the two
//! smallest sizes usable there.
//!
//! The layout is the classic one: a bottom-up 8bpp DIB whose height is doubled
//! to cover the 1-bit AND mask that follows the color data.
...
/// A full table is written even when fewer colors are used: this entry exists
/// for old readers, so it is not the place to be clever about `biClrUsed`.
...
/// Build a palette of at most [`PALETTE_LEN`] colors covering the opaque pixels,
/// exactly when they are few enough and by median cut when they are not.
...
fn average(b: &ColorBox) -> [u8; 3] {
    ...
    for (ch, slot) in out.iter_mut().enumerate() {
        let sum: u64 = b.iter().map(|(c, n)| u64::from(c[ch]) * u64::from(*n)).sum();
